...
See [Install in FiveM](../lua-resource/install). Short version: drop the `nexora-dashboard` folder into your server's `resources/`, set the `DashboardURL` and `ApiKey` in its `config.lua`, then `ensure nexora-dashboard` in your `server.cfg`.
